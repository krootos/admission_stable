...
<option value="TYPE22"<?php if(isset($_REQUEST["cat"])){ if($_REQUEST["cat"] == "TYPE22"){  echo"selected"; }} ?>
     >22 : ระดับชั้นที่สมัครเสร็จสิ้น</option>
<option value="TYPE23"<?php if(isset($_REQUEST["cat"])){ if($_REQUEST["cat"] == "TYPE23"){  echo"selected"; }} ?>
     >23 : ระดับชั้นที่สมัครเสร็จสิ้น</option>
<option value="TYPE25"<?php if(isset($_REQUEST["cat"])){ if($_REQUEST["cat"] == "TYPE25"){  echo"selected"; }} ?>
     >25 : ระดับชั้นที่สมัครเสร็จสิ้น</option>
<option value="TYPE26"<?php if(isset($_REQUEST["cat"])){ if($_REQUEST["cat"] == "TYPE26"){  echo"selected"; }} ?>
...
<td width="64%">
     ...
     <button class="btn btn-info" name="btnSearch" id="btnSearch" type="submit" value="btnSearch">Search</button>
      ...
      <?php if(isset($_SESSION["cat"])){ echo "href=\"index.php?btnAll=yes\""; }else{
       ...
       ?>>ทั้งหมด</a>
<br><br>
        <a class="btn btn-default" href="index.php?le=1&op=นักเรียนในเขตพื้นที่บริการ"> ม.1 ในเขต</a>
        <a class="btn btn-default" href="index.php?le=1&op=นักเรียนทั่วไป"> ม.1 ทั่วไป</a>
        <a class="btn btn-default" href="index.php?le=6"> ม.1 ทั้งหมด</a>
        <a class="btn btn-default" href="index.php?le=7"> ม.4 ทั่วไป</a>
        <a class="btn btn-default" href="index.php?le=5"> ม.4 โควต้า</a>
...
if(isset($_SESSION["cat"],$_SESSION["Search"]) && $_SESSION["Search"] != "") {
        ...
        elseif (strcmp($_SESSION["cat"], "TYPE22") == 0) {
                ...
                INNER JOIN sas_studentdata as sd
                ON sr.RegisNaID = sd.NID
                WHERE sr.RegisStatus = '1' and sd.DAYCOME = '22'  and sd.TYPE = '".$_SESSION["Search"]."'
                ORDER BY sr.RegisID DESC "; // คำสั่งค้นหา
        }
        ...
        elseif (strcmp($_SESSION["cat"], "TYPE26") == 0) {
            $query = "SELECT RegisID, RegisNO, RegisPWD, RegisLLog, RegisNaID, RegisStatus, Daypush, sd.NID, sd.TYPE, sd.OPTIONS, sd.PLAN1, sd.SNAME, sd.FNAME, sd.LNAME, sd.DAYCOME
            FROM sas_register as sr
            INNER JOIN sas_studentdata as sd
            ON sr.RegisNaID = sd.NID
            WHERE sr.RegisStatus = '1' and sr.Daypush = '26'  and sd.TYPE = '".$_SESSION["Search"]."'
            ORDER BY sr.RegisID DESC "; // คำสั่งค้นหา
        }
        elseif (strcmp($_SESSION["cat"], "TYPE27") == 0) {
                ...
                INNER JOIN sas_studentdata as sd
                ON sr.RegisNaID = sd.NID
                WHERE sr.RegisStatus = '1' and sr.Daypush = '27'  and sd.TYPE = '".$_SESSION["Search"]."'
                ORDER BY sr.RegisID DESC "; // คำสั่งค้นหา
        }
 ...
 elseif(isset($_SESSION["cat"],$_SESSION["Search"]) && $_SESSION["Search"] == "") {
            if (strcmp($_SESSION["cat"], "TYPE22") == 0) {
                $query = "SELECT RegisID, RegisNO, RegisPWD, RegisLLog, RegisNaID, RegisStatus, sd.NID, sd.TYPE, sd.OPTIONS, sd.PLAN1, sd.SNAME, sd.FNAME, sd.LNAME, sd.DAYCOME
                FROM sas_register as sr
                INNER JOIN sas_studentdata as sd
                ON sr.RegisNaID = sd.NID
                WHERE sr.RegisStatus = '1' and sd.DAYCOME = '22'
                ORDER BY sr.RegisID DESC "; // คำสั่งค้นหา
        }
        elseif (strcmp($_SESSION["cat"], "TYPE23") == 0) {
               $query = "SELECT RegisID, RegisNO, RegisPWD, RegisLLog, RegisNaID, RegisStatus, Daypush, sd.NID, sd.TYPE, sd.OPTIONS, sd.PLAN1, sd.SNAME, sd.FNAME, sd.LNAME, sd.DAYCOME
        ...
        elseif (strcmp($_SESSION["cat"], "TYPE26") == 0) {
            $query = "SELECT RegisID, RegisNO, RegisPWD, RegisLLog, RegisNaID, RegisStatus, Daypush, sd.NID, sd.TYPE, sd.OPTIONS, sd.PLAN1, sd.SNAME, sd.FNAME, sd.LNAME, sd.DAYCOME
             ...
             INNER JOIN sas_studentdata as sd
             ON sr.RegisNaID = sd.NID
             WHERE sr.RegisStatus = '1' and sr.Daypush = '26'
             ORDER BY sr.RegisID DESC "; // คำสั่งค้นหา
     }
        elseif (strcmp($_SESSION["cat"], "TYPE27") == 0) {
               $query = "SELECT RegisID, RegisNO, RegisPWD, RegisLLog, RegisNaID, RegisStatus, Daypush, sd.NID, sd.TYPE, sd.OPTIONS, sd.PLAN1, sd.SNAME, sd.FNAME, sd.LNAME, sd.DAYCOME
                ...
                INNER JOIN sas_studentdata as sd
                ON sr.RegisNaID = sd.NID
                WHERE sr.RegisStatus = '1' and sr.Daypush = '27'
                ORDER BY sr.RegisID DESC "; // คำสั่งค้นหา
        }
}
...
elseif(isset($_GET["le"]) && strcmp($_GET["le"], "7") == 0){
